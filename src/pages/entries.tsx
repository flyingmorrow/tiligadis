...
export default function LayoutAuthenticated({ children }: Props) {
  ...
  const handleDownloadPDF = async () => {
    ...
    // const fontFileURL = new URL('../css/fonts/Arialbd.ttf', import.meta.url); // Construct the font file URL
    // const fontFile = await fetch(fontFileURL);
    // const fontArrayBuffer = await fontFile.arrayBuffer();
  
    // doc.addFileToVFS('Arialbd.ttf', fontArrayBuffer); // Add font using ArrayBuffer
    // doc.addFont('Arialbd.ttf', 'Arialbd', 'normal');
    // doc.setFont('Arialbd');
  ...
  const handleXLSX = async () => {
    ...
    Object.entries(entries).map(([key, value]: any) => {
      if (checkedItems.includes(value.id)) {
        ...
        if (value.productsList?.[0]?.brandName)
          tempOOS = value.productsList?.[0]?.brandName + ',';
        if (value.productsList?.[1]?.brandName)
          tempOOS += value.productsList?.[1]?.brandName + ',';
        if (value.productsList?.[2]?.brandName)
          tempOOS += value.productsList?.[2]?.brandName + ',';
        if (value.productsList?.[3]?.brandName)
          tempOOS += value.productsList?.[3]?.brandName + ',';
        if (value.productsList?.[4]?.brandName)
          tempOOS += value.productsList?.[4]?.brandName + ',';
        if (value.productsList?.[5]?.brandName)
          tempOOS += value.productsList?.[5]?.brandName + ',';
        if (value.productsList?.[6]?.brandName)
          tempOOS += value.productsList?.[6]?.brandName;

  const obj = {
    date: value.date,
    user: value.username,
    westCode: value.store.westCodes,
    customer_Id: value.store.customerName,
    concessionaire_name: value.store.concessionaireName,
    street: value.store.street,
    city: value.store.city,
    region: value.store.region_Details,
    PC: value.store.pc,
    chain: value.store.chain,
    ηρείται_το_πλανόγραμμα: value.question1,
    χει_καλή_εικόνατο_σημείο: value.question2,
    Καταγραφή_OOS_Προϊόντων: tempOOS,
...
return (
  <div className={`${darkMode ? 'dark' : ''}  bg-white overflow-hidden lg:overflow-visible`}>
    ...
    <Toaster />
    <div
      className={`${layoutAsidePadding} ${isAsideMobileExpanded ? '' : ''
        ...
        <Table striped className="border shadow-sm" responsive hover ref={elementRef}>
          <thead>
            <tr className="border shadow" style={{ fontSize: '9px', fontWeight: 'bold' }}>
              ...
              <th className="border shadow">Concessionaire Name</th>
              <th className="border shadow">Street</th>
              <th className="border shadow">City</th>
              <th className="border shadow">Region</th>
              <th className="border shadow">PC</th>
              ...
              resultItems.map(([key, value]: any) => {
                return (
                  <tr className="shadow " style={{ fontSize: '10px' }} key={value.id}>
                    ...
                    <td className="border ">{value.store.westCodes}</td>
                    <td className="border ">{value.store.customerName}</td>
                    <td className="border ">{value.store.concessionaireName}</td>
                    <td className="border ">{value.store.street}</td>
                    <td className="border ">{value.store.city}</td>
                    <td className="border ">{value.store.region_Details}</td>
                    <td className="border ">{value.store.pc}</td>
